...
def tile_image(img, patch_size, stride, out_dir, scene):
    ...
    for corner in product(list(range(0, heigth - patch_size, int(patch_size * stride))) + [heigth - patch_size], 
                          list(range(0, width - patch_size, int(patch_size * stride))) + [width - patch_size]):
        left, down = corner
