...
def parse_args():
    ...
    parser.add_argument(
        "--input_folder", "-i", type=str, help="input folder with images to tile"
    )
    parser.add_argument(
        "--output_folder",
        "-o",
        type=str,
        default="test_set",
        help="destination folder for crops",
    )
    parser.add_argument(
        "--patch_size",
        "-p",
        type=int,
        default=256,
        help="patch size to crop, must match model input requirements",
    )
    parser.add_argument(
        "--stride",
        "-s",
        type=float,
        default=1.0,
        help="stride for sliding window, multiplies patch_size",
    )
