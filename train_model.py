...
def main():
    ...
    if args.segmentation:
        ...
        if args.finetune:
            ...
            pretrained_dict = torch.load('checkpoints/Resnet34_best.pth', map_location=torch.device('cpu'))
        ...
        model_trainer = TrainerSegmentation(model, device=f'cuda:{args.device}', patch_size=args.patch_size,
        ...
        model_trainer = TrainerClassification(model, device=f'cuda:{args.device}', patch_size=args.patch_size,
                                              batch_size=(args.batch_size, args.batch_size * 2), epochs=args.epochs,
                                              lr=args.lr, data_folder=args.training_set, model_name=model_name)
